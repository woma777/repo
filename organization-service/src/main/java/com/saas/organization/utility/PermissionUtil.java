...
import com.saas.organization.client.AuthServiceClient;
import com.saas.organization.config.RoleConverter;
...
public class PermissionUtil {
    ...
    public boolean hasPermission(UUID tenantId, String resourceName) {
        ...
        for (GrantedAuthority authority : userRoles) {
            Tenant tenant = validationUtil.getTenantById(tenantId);
    ...
    public boolean isTenantEmployee(UUID tenantId) {

        String userTenantId = securityUtil.getTenantId();
        if(userTenantId.equals(tenantId.toString())) {
            return true;
        }
